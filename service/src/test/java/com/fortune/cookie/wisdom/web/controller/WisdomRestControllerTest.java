...
public class WisdomRestControllerTest {
	...
	public void setUp() {
		...
		underTest = new WisdomRestController(service, transformer, linkFactory);
	}
	...
	public void testGetCategoriesShouldCallServiceAndTransformerAndLinkFactory() {
		...
		assertThat("response type check", response.get(0), instanceOf(CategoryResponse.class));
		assertThat("response contains the correct category", ((CategoryResponse) response.get(0)).getCategory(),
				equalTo(category));
	}
	...
	public void testGetWisdomsBzCategoriesShouldCallServiceAndTransformerAndLinkFactory() {
		...
		WisdomResponse wisdomResponse = new WisdomResponse(1L, "test_category", "test_text");
		...
		// WHEN
		List<AbstractResponse> response = underTest.getWisdomsByCategories("test_category");
		// THEN
		assertThat("size of response list", response.size() == 1);
		assertThat("response type check", response.get(0), instanceOf(WisdomResponse.class));
		assertThat("response contains the correct wisdom", ((WisdomResponse) response.get(0)), equalTo(wisdomResponse));
	}
